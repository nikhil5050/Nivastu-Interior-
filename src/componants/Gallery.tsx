...
const Gallery = () => {
  ...
  // 2. Tell the state it can be a GalleryItem OR null
  ...
  return (
    <section className="py-10 bg-[#F5F1EF] text-[#7A4A4D] min-h-screen">
      <div className="max-w-7xl mx-auto px-6">
        
        {/* Header */}
        <div className="text-center mb-16">
          <h2 className="text-[#7A4A4D] text-4xl font-extralight tracking-[0.3em] uppercase mb-8">Portfolio</h2>
          
          {/* Filter Tabs */}
          <div className="flex justify-center space-x-8 border-b border-[#B08682]/30 pb-4">
            {categories.map((cat) => (
              <button
                key={cat}
                onClick={() => setFilter(cat)}
                className={`relative text-sm uppercase tracking-widest transition-colors duration-300 ${
                  filter === cat ? "text-[#7A4A4D]" : "text-[#B08682] hover:text-[#7A4A4D]"
                }`}
              >
                {cat}
                {filter === cat && (
                  <motion.div 
                    layoutId="underline" 
                    className="absolute -bottom-[17px] left-0 right-0 h-[2px] bg-[#B08682]" 
                  />
                )}
              </button>
            ))}
          ...
          <AnimatePresence mode='popLayout'>
            {filteredImages.map((img) => (
              <motion.div
                ...
                className="relative group aspect-square cursor-pointer overflow-hidden bg-[#EBE0D8] rounded-2xl"
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[#7A4A4D] via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6">
                  <p className="text-[#B08682] text-xs uppercase tracking-widest mb-1 font-bold">{img.category}</p>
                  <h3 className="text-[#7A4A4D] text-xl font-light">{img.title}</h3>
                </div>
                
                <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                  <Plus className="text-[#7A4A4D] w-6 h-6" />
                </div>
              </motion.div>
            ))}
          </AnimatePresence>
      ...
      <AnimatePresence>
        {selectedImg && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex items-center justify-center bg-[#F5F1EF]/95 backdrop-blur-md p-4"
            onClick={() => setSelectedImg(null)}
          >
            <button className="absolute top-8 right-8 text-[#7A4A4D] hover:text-[#B08682] transition-colors">
              <X size={32} />
            </button>
            ...
            >
              ...
              <div className="mt-6 text-center">
                <span className="text-[#B08682] uppercase tracking-[0.2em] text-sm font-bold">{selectedImg.category}</span>
                <h2 className="text-[#7A4A4D] text-2xl font-light mt-2">{selectedImg.title}</h2>
              </div>
            </motion.div>
          </motion.div>
        )}
